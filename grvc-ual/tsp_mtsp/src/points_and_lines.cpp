...
#define A 14 // NUMERO DE PUNTOS DE LA TRAYECTORIA (de la matriz "puntos")
...
/*
void repeticion(void){
	
	// EMPIEZA REPETICION
 
 // VARIABLES "INVENTADAS POR AHORA"
 fin=0; // por poner algo, al menos de momento
 //
 salir=0;
 nodo_1=padre;  // en matlab pone nodo_desglosable[INDICE];
 distancia=nodo_1.distancia_recorrida;
 
 if(es_ultimo(padre,puntos2,fin)>0){
 // no desgloso el nodo si está en el último nivel que quiero
	...
	// y lo defino completamente	
		...
		distancia=salida_dn.a[1].distancia_recorrida;
	...
	for(int ii=0;ii<salida_dn.a.size();ii++){
		...
		c[0]=salida_dn.a[ii].punto_act; c[1]=fin;
		r1=cut_matrix(puntos2,c[0],-1);
	    r2=cut_matrix(puntos2,c[1],-1);
		auxx=norma(resta(r1,r2));
...
// SECCION DE "REPETICION" ENCARGADA DE REFRESCAR ÍNDICE ENTRE ITERACIONES
	...
	for (int i=0; i<nodo_desglosable.size();i++){
		...
		if((nodo_desglosable[i].indicador>0) && (componente_rep(v,nodo_desglosable[i].indicador)==0) 
...
// APLICO DOS VECES LA FUNCION "devuelve hijo" PARA LLEGAR AL NIETO
/*
padre.nivel=1;
padre.definido=0;
...
salida=devuelve_hijo(padre,puntos2,0,1); // Ojo con punto2: PUTADA 


ROS_INFO("DATOS DEL PADRE MADURO: ");
ROS_INFO("NIVEL: %d",salida.p.nivel);
ROS_INFO("DEFINIDO: %d",salida.p.definido);
ROS_INFO("INDICADOR: %d",salida.p.indicador);

ROS_INFO("PUNTO SIGUIENTE: ");
for (int i = 0; i < salida.p.punto_sig.size(); i++) {
            cout << salida.p.punto_sig[i] << " ";
        }   
  ROS_INFO("\n");

ROS_INFO("CAMINO ACTIVO: ");
for (int i = 0; i < salida.p.camino_activo.size(); i++) {
            cout << salida.p.camino_activo[i] << " ";
        }   
  ROS_INFO("\n");
...
ROS_INFO("DISTANCIA RECORRIDA: %f",salida.p.distancia_recorrida);
ROS_INFO("VIVO: %d",salida.p.vivo);
ROS_INFO("ESCOGE: ");
for (int i = 0; i < salida.p.escoge.size(); i++) {
            cout << salida.p.escoge[i] << " ";
        }   
  ROS_INFO("\n");
  
ROS_INFO("RECORRIDO: ");
for (int i = 0; i < salida.p.recorrido.size(); i++) {
            cout << salida.p.recorrido[i] << " ";
        }   
  ROS_INFO("\n");

ROS_INFO("PUNTO ACTUAL: %f \n",salida.p.punto_act);

ROS_INFO("DATOS DEL HIJO: ");
ROS_INFO("NIVEL: %d",salida.h.nivel);

ROS_INFO("RECORRIDO: ");
for (int i = 0; i < salida.h.recorrido.size(); i++) {
            cout << salida.h.recorrido[i] << " ";
        }   
  ROS_INFO("\n");

ROS_INFO("PUNTO SIGUIENTE: ");
for (int i = 0; i < salida.h.punto_sig.size(); i++) {
            cout << salida.h.punto_sig[i] << " ";
        }   
  ROS_INFO("\n");

ROS_INFO("INDICADOR: %d",salida.h.indicador);
ROS_INFO("DEFINIDO: %d",salida.h.definido);
ROS_INFO("DISTANCIA RECORRIDA: %f",salida.h.distancia_recorrida);
ROS_INFO("PUNTO ACTUAL: %f ",salida.h.punto_act);

salida2=devuelve_hijo(salida.h,puntos2,salida.h.distancia_recorrida,salida.contador); 

ROS_INFO("DATOS DEL PADRE MADURO 2: ");
ROS_INFO("NIVEL: %d",salida2.p.nivel);
ROS_INFO("DEFINIDO: %d",salida2.p.definido);
ROS_INFO("INDICADOR: %d",salida2.p.indicador);

ROS_INFO("PUNTO SIGUIENTE: ");
for (int i = 0; i < salida2.p.punto_sig.size(); i++) {
            cout << salida2.p.punto_sig[i] << " ";
        }   
  ROS_INFO("\n");

ROS_INFO("CAMINO ACTIVO: ");
for (int i = 0; i < salida2.p.camino_activo.size(); i++) {
            cout << salida2.p.camino_activo[i] << " ";
        }   
  ROS_INFO("\n");
  
ROS_INFO("CAMINO: ");
for (int i = 0; i < salida2.p.camino.size(); i++) {
            cout << salida2.p.camino[i] << " ";
        }   
  ROS_INFO("\n");
  
ROS_INFO("DISTANCIA RECORRIDA: %f",salida2.p.distancia_recorrida);
ROS_INFO("VIVO: %d",salida2.p.vivo);
ROS_INFO("ESCOGE: ");
for (int i = 0; i < salida2.p.escoge.size(); i++) {
            cout << salida2.p.escoge[i] << " ";
        }   
  ROS_INFO("\n");
  
ROS_INFO("RECORRIDO: ");
for (int i = 0; i < salida2.p.recorrido.size(); i++) {
            cout << salida2.p.recorrido[i] << " ";
        }   
  ROS_INFO("\n");
