...
// EMPIEZA REPETICION 
 ...
 // no desgloso el nodo si está en el último nivel que quiero
	...
	for(int ii=0;ii<salida_dn.a.size();ii++){
		...
		// DEPURACION DE DATOS (INICIO)
		
		     ROS_INFO(" PUNTOS2 (PUNTOS EN REALIDAD): ");
for (int i = 0; i < puntos2.size(); i++) {
	cout << "PUNTO " << i << ": " ;
	for (int j = 0; j < puntos2[i].size(); j++) {
            cout << puntos2[i][j] << " ";
		}
		ROS_INFO("\n");
        }   
  ROS_INFO("\n");
		...
		// PRUEBO CON DARLE VALOR INICIAL A r1...
		cout << "R1: " ;
		for (int kk=0;kk<r1.size();kk++){
			r1[kk]=0;
			cout << r1[kk] << " ";
		}
		ROS_INFO("\n");
		
		// DEPURACION DE DATOS (FIN)
